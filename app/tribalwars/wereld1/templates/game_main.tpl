...
<table class="vis">
	<tr>
		<th width="250">Order{$lang->get("build")}</th>
	...
	<tr {if $id==0}class="lit"{/if}>
		...
		<td><a href="javascript:ask('Are you sure you want to cancel this?', 'game.php?village={$village.id}&amp;screen=main&amp;action=cancel&amp;id={$do_build.$id.r_id}&amp;mode={$mode}&amp;h={$hkey}')">{$lang->get("cancel")}</a></td>
	</tr>
	{/foreach}
	{if $num_do_build > 2}
		...
		<td align="center">
			<table cellpadding="0" rowspacing="0" cellspacing="0">
				<tr>
					...
					<td align="center"><a href="javascript:ask('It costs you extra resources to build. Do you want to continue?', 'game.php?village={$village.id}&amp;screen=main&amp;action=build&amp;id={$dbname}&amp;force&amp;h={$hkey}')"><img src="{$config.cdn}/graphic/icons/plus.png"></a></td>
				</tr>
			</table>
		</td>
							{else}

{if empty($view)}
	...
	<tr><th>Betreff</th><th width="160">Absender</th><th width="140">Gesendet</th></tr>
		...
		<table align="left"><tr>
		<td><input type="submit" value="Clear" name="del" /></td>
...
{else}
	{if empty($error)}
		...
		<td align="center" width="25%"><a href="game.php?village={$village.id}&amp;screen=mail&amp;action=del&amp;id={$mail.id}&amp;mode=in&amp;h={$hkey}">L�schen</a></td>
		</tr>
		</table>
		...
		<tr><td>Receiver</td><td><a href="game.php?village={$village.id}&amp;screen=info_player&amp;id={$mail.to_id}">{$mail.to_username}</a></td></tr>
		...
		<td align="center">{if $mail.from_id==-1}Block sender{else}<a href="game.php?village={$village.id}&amp;screen=mail&amp;mode=block&amp;action=block_id&amp;id={$mail.from_id}&amp;h={$hkey}">Absender blockieren</a>{/if}</td>
		</tr>
		</table><br />
	{/if}
{/if}
...
{if isset($simulate) && $simulate}
	...
	{if $simulate_values.others.def_wall!=$simulate_values.others.new_wall}
		<tr><th>Schaden durch Rammb�cke:</th><td>Wall damaged from level <b>{$simulate_values.others.def_wall}</b> to level <b>{$simulate_values.others.new_wall}</b></td></tr>
	{/if}
	
	{if $simulate_values.others.def_building!=$simulate_values.others.new_building}
		<tr><th>Damage caused by catapults:</th><td>Building damaged from level <b>{$simulate_values.others.def_building}</b> auf Level <b>{$simulate_values.others.new_building}</b></td></tr>
	{/if}
	</table>
{/if}
...
<form action="game.php?village={$village.id}&amp;screen=place&amp;mode=sim" method="post" name="units">
	...
	<table class="vis">
		...
		<tr>
			<td>Katapultziel Geb�udelevel</td>
